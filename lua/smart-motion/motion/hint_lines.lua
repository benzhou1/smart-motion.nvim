...
local lines_extractor = require("smart-motion.extractors.lines")
local log = require("smart-motion.core.log")

local M = {}

--- Public method: Hints lines from the possible jump targets in a given direction.
...
function M.run(direction, hint_position, ignore_whitespace)
	...
	--
	local lines_collector_generator = lines_collector.init()
	if not lines_collector_generator then
		log.debug("No lines to search - exiting early")
		return
	end

	--
	-- Extract Lines (Streaming)
	--
	local lines_extractor_generator = lines_extractor.init(lines_collector_generator)
	if not lines_extractor_generator then
	...
	-- Build Jump Targets
	--
	targets.get_jump_targets(ctx, cfg, motion_state, lines_extractor_generator)
