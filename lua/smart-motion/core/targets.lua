...
function M.get_jump_targets_for_word(ctx, cfg, motion_state)
	...
	if motion_state.direction == consts.DIRECTION.AFTER_CURSOR then
		for line_index, line_text in ipairs(motion_state.lines) do
...
function M.get_jump_targets(ctx, cfg, motion_state)
	local jump_targets = {}

	log.debug("Fetching jump targets for target_type: " .. motion_state.target_type)

	if motion_state.target_type == consts.TARGET_TYPES.WORD then
		jump_targets = M.get_jump_targets_for_word(ctx, cfg, motion_state)
	end
